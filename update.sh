...
echo -e "${BLUE}**Вставьте ссылку на вашу подписку (вы можете её найти в боте, по кнопке \"Доступ к VPN - Поделиться подпиской\"):**${RESET}" >&2
read SUB_URL < /dev/tty

if [ -z "$SUB_URL" ]; then
    echo -e "${RED}**Ошибка: ссылка не введена!**${RESET}" >&2
    exit 1
fi
